...
class Option(object):
    ...
    def is_valid_single(self, value):
        ...
        if type(self.allowed_values) == tuple:
            ...
            if (
                type(value) != type(self.value)
                and type(self.value) not in MultiContainerTypes
            ):
    ...
    def as_documentation_string(self, section=None):
        ...
        strval.append(
            "\t<td>%s</td>" % ", ".join([str(s) for s in self.valid_input()])
        )
    ...
    def convert(self, value):
        ...
        if svt == type(value):
            ...
            return value
        if (
            type(self.allowed_values) == tuple
            and value in self.allowed_values
        ):
            ...
            if self.delimiter is None:
                if type(self.allowed_values) == tuple:
                    ...
                    for sep in [' ', ',', ':', ';', '/', '\\', None]:
                        ...
                        if (
                            test_tuple[0] == str(v0)
                            and test_tuple[1] == str(v1)
                            and len(test_tuple) == 2
                        ):
                            break
    ...
    def is_boolean(self):
        ...
        if (
            ...
            and type(self.allowed_values[0]) == bool
        ):
            return True
        return False
...
class OptionsClass(object):
    ...
    def _add_missing(self, out, written, sect, vi, label=True):
        ...
        for opt in self.options_in_section(sect):
            if not (sect, opt) in written and self.get(
                sect, opt
            ) != self.default(sect, opt):
                if label:
                    out.write('[')
                    out.write(sect)
                    out.write("]\n")
    ...
    def _report_deprecated_error(self, sect, opt):
        print(
            ("Warning: option %s in section %s is deprecated" % (opt, sect)),
            file=sys.stderr,
        )
